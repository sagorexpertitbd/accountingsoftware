...
@section('title', 'Receive Voucher - '.$settingsinfo->company_name.' - '.$settingsinfo->soft_name.'')
...
@if($set_currency->currency_position==2){{$set_currency->symbol}}@endif 
  ...
  @section('js')
    ...
    $(document).ready(function() {
        dataTableLoad({
            curUrl: "{{route('Accounts.receive_voucher.index')}}",
            addUrl: "{{route('Accounts.receive_voucher.create')}}"
        });
    });
    </script>
  @endsection